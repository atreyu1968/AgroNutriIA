...
# ----------------------------------------------------------------------------
log "Cargando el catálogo base de fertilizantes (solo si está vacío)"
# Abonos habituales en fertirrigación de platanera (deploy/seed-fertilizers.sql).
# Solo se insertan cuando el catálogo está vacío, para no pisar productos
# añadidos o editados por el usuario en reinstalaciones/actualizaciones.
sudo -u postgres psql -v ON_ERROR_STOP=1 -d "${DB_NAME}" -f "$APP_DIR/deploy/seed-fertilizers.sql"
echo "Catálogo de fertilizantes listo."
